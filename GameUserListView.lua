...
function newListItem(params)
        local gameListData = params.gameListData
        ...
        thisItem.id = id
        thisItem.gameListData = gameListData
        thisItem.onRelease = onRelease          
        thisItem.top = top
        thisItem.bottom = bottom
 
        local t = callback(gameListData)
        thisItem:insert( t )
...
t.y = 24 
    return t
        ...
        if cat then         
        	...
        	for i=1, #gameListData do
        		if gameListData[i][cat] ~= prevCat then
        			table.insert(catTable, gameListData[i][cat])
        ...
        while true do
	        ...
	        --iterate over the gameListData and add items to the list view
	        for i=1, #gameListData do
	        	if gameListData[i][cat] == h then  
 	                local thisItem = newListItem{
	                    gameListData = gameListData[i],
	                    default = default,
	                    over = over,
	                    onRelease = onRelease,
	                    top = top,
      ...
      Runtime:removeEventListener( "enterFrame", showHighlight )
		...
		function listView:scrollTo(yValue, timeValue)
            local timeVal = timeValue or 400
            local yVal = yValue or 0
